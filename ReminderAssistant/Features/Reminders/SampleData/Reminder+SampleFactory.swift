...
/// `date` には「昨日」「今日」「明日」、または正の半角整数を使った「3日前」「3日後」の形式を指定する。
/// `time` には「9:30」「09:30」のような `H:mm` または `HH:mm` 形式を指定し、時は0〜23、分は00〜59の範囲とする。
/// 全角数字、符号、余分な空白を含む値、および「0日前」「0日後」は指定できない。
/// `time` を指定する場合は `date` も指定する。日時を指定しない場合は、`.init(date: nil, time: nil)` を使用する。
