...
/// ## アクターとしての責務
///
/// リポジトリをアクターとすることで、`EventKit`へのアクセスと操作の待機列をUIを担う`MainActor`から分離している。また、`EventKit`の変更通知を監視するオブザーバートークンもこのアクターが所有し、インスタンスの破棄時に解除する。
///
/// ## 操作の直列化
///
...
public final actor EventKitReminderRepository: ReminderRepository {
    ...
    /// 現在の`Task`がキャンセルされている場合に、``ReminderRepositoryError/cancelled``を投げる。
    ...
    /// リマインダーを保存する。保存時のエラーは``ReminderRepositoryError``のエラーへ変換する。
    private func save(_ reminder: EKReminder) throws(ReminderRepositoryError) {
        do {
